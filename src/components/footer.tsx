...
export function Footer() {
  return (
    <footer className="mt-auto border-t border-[#e5e7eb] bg-[#f5f5f7] text-[#111827]">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 md:grid-cols-4 lg:px-8">
        <div className="md:col-span-2">
          ...
          <p className="mt-5 text-sm text-[#475467]">License: {siteDetails.license}</p>
          <p className="mt-1 text-sm text-[#475467]">Address: {siteDetails.address}</p>
        </div>
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-[#475467]">
            {loanProducts.map((loan) => (
              <li key={loan.slug}>
                <Link className="hover:text-[#111827]" href={`/loans/${loan.slug}`}>
                  {loan.name}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-[#475467]">
            {footerLinks.map((link) => (
              <li key={link.href}>
                <Link className="hover:text-[#111827]" href={link.href}>
                  {link.label}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t border-[#e5e7eb]">
        <div className="mx-auto flex max-w-7xl flex-col gap-2 px-4 py-5 text-xs text-[#667085] sm:flex-row sm:justify-between sm:px-6 lg:px-8">
